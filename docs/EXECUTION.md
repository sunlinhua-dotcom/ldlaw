...
| P1 知识层 | T1.1 / T1.4 / T1.7 ✅；T1.2 / T1.3 / T1.5 / T1.6 🔶；T1.8 ⬜ | 法规 15 部 719 条待 G1；词条 30 条待 G2；案例 24 件待 G闸（官方源采集，verified=0）|
| P2 检索精度 | T2.7 / T2.4 / T2.8 ✅；T2.1 ⬜（见状态注）；T2.6 🔶；T2.3/T2.2/T2.5 ⬜ | 评测满分(起步集)；多轮补全闭环；T2.8 放开作答（超纲给方案+分档标出处，编造率仍=0）|
| P3 界面双端 | T3.1 ✅；T3.3 🔶（UI 就绪、数据 0）；T3.2 ⬜；T3.4 / T3.5 属 M1 | 375px 横向溢出已修（2026-06-13）；法库页加六道防线可信架构 SVG 图、四表移动端卡片化（2026-06-16） |
...
**T2.8 放开作答（拒答 → 开放·分档标注出处）✅ 2026-06-17**（产品决策：要"不硬拒、给方案"）：
概念/开放问题在 entry_hit / rag 都不命中时，不再硬拒，改走 `open_answer`——模型基于通用
法律知识给可操作方案 + 步骤，**法条逐条回库核验**：库内有的进 `citations`（真·已核验），
库内无的进 `ai_refs`（前端虚线框标「AI 标注·未核验」，绝不冒充库内）。**守两条底线**：
①涉钱仍只走 calculators（OPEN_SYS 禁止给金额数字，money intent 永不进 open）；
②编造法条隔离进 ai_refs，评测"编造引用率"只统计 `citations`（`run_eval.py:68` 不扫 ai_refs）。
Python `pipeline.open_answer` + JS `functions/_pipeline.js openAnswer` 双侧同步；前端 `route:'open'`
两档渲染。评测金标准随规格更新：`open-pet-6`/`open-strategy-8` 由"应拒答"改为"开放作答(不硬拒+不编造)"，
...
**聚焦杭州+上海 + UI 升级 ✅ 2026-06-17**（用户需求）：①产品聚焦——`REGIONS=["杭州","上海"]`（两端）+
前端 `FOCUS_REGIONS` 过滤所有地区选择器、**默认杭州**，其余地区数据保留在库不开放选择。②精准数据
（官方源查证、verified=0 待 G1，见记忆 `ldlawq-hz-sh-data.md`）：上海社平 12,434/封顶 37,302、上海最低 2,740；
杭州社平 13,931/封顶 41,794（市级在岗职工档，**口径司法分歧、默认在岗职工、待律师 G1 拍板**，严禁误用浙江全省 8,433）、
杭州最低 2,660；新增浙江省企业工资支付管理办法（38条）+ 浙江省劳动人事争议调解仲裁条例（48条），
法条 939→1025。③UI 升级——法条引用做成**带层级图形徽章**（法律/行政法规/司法解释/地方/文件 各图标+色）
的清晰卡片、结论朱砂边栏 headline、分段标签带色条；移动端豆包式纯对话。线上实测：杭州封顶 229,861.5、
上海 205,161（精准参数生效）；测试 30 绿、评测编造 0%。**缺口**：上海市企业工资支付办法全文官方源仍未找到（MISSING.md）。
...
> **状态注（06-13）**：批次 1 共 14 条已入库（总 16 条），覆盖未签合同二倍工资/试用期/加班费/年休假/竞业/仲裁时效/三期/医疗期/违法解除2N/调岗/规章制度/裁员/无固定期限/封顶口径，status=in_review，引用全过存在性校验。match_entry 去 N+1 硬编码改整词关键词匹配（精度闸：无整词命中不进 entry_hit，超纲问题正确落拒答）。审核清单 scripts/gen_review_checklist.py → data/review/entries_review.md 待律师 G2。**批次 2 再加 14 条补满 30**（工伤/社保/劳务派遣/非全日制/拖欠工资/最低工资/被迫解除/服务期/代通知金等）。评测四指标 100%（起步集，非泛化证明）。
- **前置**：T1.2、T1.3
- **动作**：
...
### T2.1 FTS5 + jieba 分词检索 ⬜（暂缓，见状态注）
> **状态注（06-13）**：当前是 FTS5 二元组召回 + 重叠精排 + 地区过滤，可用。升级 jieba **暂缓**，两个原因：①架构权衡——检索 query 分词在请求时发生，jieba 上 runtime 会破 server.py 纯标准库不变量（需先定：接受 jieba 为 server 依赖 / 还是 query 侧保持标准库分词）；②起步评测集已 100%，jieba 召回收益此刻量不出来，应先把 T2.6 扩到更难的题再上 T2.1 用评测证明增益。
- **前置**：T1.2
...
### T2.4 多轮对话（要素累积） ✅
> **状态注（06-13）**：后端 session 要素累积 + 前端 clarify 表单 chip 全部完成。clarify 改返回结构化 {field,type,label}，前端渲染地区下拉/日期/数字输入，「补全并继续」拼成 regex 可解析跟进句走同 session 合并。实测：缺要素→填表单→出计算闭环跑通（PC+375px）。顺带根治两处早先遗留的移动端横向溢出（用户长消息气泡 keep-all 不断行；.chat-main 网格子项 min-width:auto 撑破 1fr 轨道）。
- **前置**：T0.5
- **动作**：
