# LDLAWQ 执行手册（Opus 4.8 任务卡）v1.0 — 2026-06-11
...
```
打开 /Volumes/ProjectsAPFS/LDLAWQ，先读 EXECUTION.md 的 §1 全局约定，
然后执行任务卡 <任务ID>。严格按卡内"动作"清单执行，完成后运行卡内
"验收"命令并贴出结果。验收不过不许收工。完成后按 §1.2 格式提交 git。
