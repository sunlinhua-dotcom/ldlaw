...
用法：
  ...
  python3 src/verify_articles.py --approve <slug> --by 张三 [--benchmark 北大法宝/威科先行]  # 全一致才置 verified=1

边界：--approve 只有在该法规 0 差异 0 缺失时才放行；有差异先人工核对修 seed。
对标：自动快照以 flk.npc.gov.cn 为准；北大法宝/威科先行 为付费权威库，G1 人工比对后用
  --benchmark 留痕（默认值见 DEFAULT_BENCHMARK），不自动抓取（ToS + 合规）。
"""
...
# G1 核验对标基准：自动抓取/快照仍以 flk.npc.gov.cn（免费官方源）为准；
# 北大法宝（pkulaw.com）/ 威科先行（law.wkinfo.com.cn）为业内权威商业库，
# 付费、登录墙，**不自动抓取**——由运营/律师在 G1 人工比对，--benchmark 留痕。
DEFAULT_BENCHMARK = "北大法宝/威科先行"
...
def cmd_approve(slug: str, by: str, manifest: dict, benchmark: str | None = None) -> int:
    seed = load_seed(slug)
    ...
    for a in seed["articles"]:
        ...
        a["verified_by"] = by
        a["verified_at"] = stamp
        a["verified_benchmark"] = bench  # 对标专业库（北大法宝/威科先行），G1 人工核对留痕
    seed["_verified_note"] = (f"经 {by} 对照官方快照（flk.npc.gov.cn）逐字核验、"
                              f"并比对 {bench} 通过 @ {stamp}（G1）")
    (LAWS / f"{slug}.json").write_text(
        json.dumps(seed, ensure_ascii=False, indent=1), encoding="utf-8")
    print(f"✓ {seed['title']}：{d['counts']['total']} 条全部置 verified=1"
          f"（核验人 {by}，对标 {bench}）")
...
def main() -> int:
    ...
    if "--approve" in argv:
        if not args:
            print("用法：--approve <slug> [--by 姓名] [--benchmark 北大法宝/威科先行]")
            return 1
        by = "operator"
        if "--by" in argv and argv.index("--by") + 1 < len(argv):
            by = argv[argv.index("--by") + 1]
        benchmark = None
        if "--benchmark" in argv and argv.index("--benchmark") + 1 < len(argv):
            benchmark = argv[argv.index("--benchmark") + 1]
        # --by / --benchmark 的值会混进 args，剔除它们
        args = [a for a in args if a not in (by, benchmark)]
        return cmd_approve(args[0], by, manifest, benchmark)
