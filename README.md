...
**产品聚焦**：仅杭州 + 上海，默认杭州（其余地区数据保留在库但不开放选择）。

| 项 | 数量 | 说明 |
|---|---|---|
| 法规 | 22 部 | 全国性 20 + 浙江 2（工资支付管理办法/劳动争议条例，杭州经地区链引用）；缺：上海市企业工资支付办法全文源、司法解释（二）|
| 法条 | 1025 条 | **全部 verified = 0**；核验工具就绪待 G1 |
| 百科词条 | 30 条 | 全部 in_review 待 G2 律师审核（清单已生成）|
| 地区参数 | 23 项 | 上海/杭州 经济补偿封顶社平 + 最低工资已填官方查证值（仍 verified=0 待 G1）；杭州封顶口径待律师拍板 |
...
问答策略（T2.8）：库内能答的优先给已核验引用；超纲不再硬拒，改「开放作答」给可操作方案，
法条**逐条回库核验分两档**——库内已有的标「库内依据」，库外的标「AI 标注·未核验」（前端虚线框，
绝不冒充已核验）。涉钱永远走计算器（开放路径禁止给金额）；编造引用率仍硬控为 0。
**模型可切换**（顶栏选 DeepSeek / Claude，凭据全在服务端）；**语音输入**（按住说话 → 豆包大模型
识别 → 文本，凭据经 CF Worker 代理不下发前端，`functions/api/asr.js`）；移动端做豆包式纯对话
（隐藏引用卡，留对话气泡 + 大号「按住说话」），桌面端保留法条依据展示。
**法条引用带官方出处超链**：每条引用卡「查看官方原文出处 ↗」直达该法规官方全文页（flk.npc.gov.cn /
gov.cn / 各地人社官网，即抓取来源），点击核对原文确保正确；库外 AI 标注法条给「去官方库核验 ↗」。
（北大法宝/威科先行无免费 API，对接需付费授权，故核验链走官方免费源——它们本就是两库的上游权威源。）
